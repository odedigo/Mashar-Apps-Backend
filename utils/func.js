 ...
 * @returns boolean
 */
export function _checkVector(params, teamData) {
  var errMsg = "";
  var infoMsg = "";

  // form data
  var vs = params.vectorSize;
  var va = params.vectorAngle;
  ...
  if (success == -1) {
    ...
    infoMsg += `<p class='vector'>(${rdl.vecSize[num]},${rdl.vecAngle[num]}°)</p>`;
  }
...
export function _reportStatus(data, team, gameCode, branchCode) {
  if (!config.app.report_status) return;
